...
describe("GET /api/articles/:article_id", () => {
  ...
  test("200: gets and article by its id with correct properties", () => {
    return request(app)
      ...
      .expect(200)
      .then(({ body }) => {
        const articleRecieved = body.article;
...
describe("GET /api/articles/:article_id/comments", () => {
  ...
  test("200: returns empty array if no comments found for article", () => {
    return request(app)
...
describe("POST /api/articles/:article_id/comments", () => {
  test("201: checks the posted comment is an object", () => {
    const data = {
  ...
  test("201: checks the posted comment has correct properties", () => {
    const data = {
      username: "butter_bridge",
      body: "this is a test body",
    };
  ...
  test("201: checks the total number of comments had gone up by 1", async () => {
    const data = {
      username: "butter_bridge",
      body: "this is a test body",
    };
    ...
    await request(app).post("/api/articles/2/comments").send(data).expect(201);
  ...
  test("use a 404: Not Found status code when `POST` contains a valid username that does not exist", () => {
    const data = {
      username: "butter_icecream",
      body: "this is a test body",
    };
    return request(app)
      .post("/api/articles/2/comments")
      ...
      .expect(404)
      .then(({ body }) => {
        expect(body.msg).toBe("Not user with this username");
      });
  });
});
...
describe("PATCH /api/articles/:article_id", () => {
  test("200: responds with an object", () => {
    ...
    return request(app)
      ...
      .expect(200)
      .then(({ body }) => {
        const articleUpdated = body.article;
        expect(typeof body).toBe("object");
        expect(articleUpdated.article_id).toBe(4);
  ...
  test("200: nothing changed when no input", () => {
    return request(app)
      .patch("/api/articles/7")
      .send({})
      .expect(200)
...
describe("GET /api/articles/:article_id and check comment count", () => {
  test("200: gets the article by ID with comment_count", () => {
    return request(app)
      .get("/api/articles/1")
      .expect(200)
      .then(({ body: { article } }) => {
